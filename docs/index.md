...
Oktopus is a vendor-agnostic control plane for agent orchestration, governance, and memory.
...
From there, four pillars — connect to your tools, compose the work, govern the execution, prove the outcome:

```mermaid
flowchart LR
    Connect["Connect<br/>any input · any output"]
    Compose["Compose<br/>build the work"]
    Govern["Govern<br/>control execution"]
    Prove["Prove<br/>evidence · memory"]
    Connect --> Compose --> Govern --> Prove
```
...
- **Portability** — runs, artifacts, and scoped memory (thread, user, team, org) live in the control plane. Context transfers across agents: switch the runtime, keep the context. *Enforced by [define-core-run-job-event-model](https://github.com/convexideas/oktopus/blob/main/openspec/changes/define-core-run-job-event-model), [define-enterprise-admin-marketplace-sessions](https://github.com/convexideas/oktopus/blob/main/openspec/changes/define-enterprise-admin-marketplace-sessions).*
